...
int main(int argc, char** argv)
{
   ...
   private_nh.param("file_directory", file_directory, std::string("/home/tommy/pcd/"));
   ROS_INFO("*** file_directory = %s ***\n", file_directory.c_str());


   private_nh.param("file_name", file_name, std::string("12673.944000000"));
   ROS_INFO("*** file_name = %s ***\n", file_name.c_str());

   pcd_file = file_directory + file_name + pcd_format;
   ROS_INFO("*** pcd_file = %s ***\n", pcd_file.c_str());

   private_nh.param("thre_z_min", thre_z_min, 0.18);
   private_nh.param("thre_z_max", thre_z_max, 5.0);
...
//   RadiusOutlierFilter(cloud_after_PassThrough, 0.2, 10);
